...
/// The UNIX timestamp which celestica timestamps start counting from.
///
/// This is essentially the `1st Jan, 2023`.
pub const CELESTICA_EPOCH: Duration = Duration::from_secs(1672534861);
...
impl HLCTimestamp {
    ...
    pub fn now(counter: u16, node: u8) -> Self {
        let duration = get_celestica_timestamp();
    ...
    #[inline]
    /// The celestica timestamp as seconds.
    ///
    ...
    ///
    /// NOTE:
    /// This adds the [CELESTICA_EPOCH] back to the duration to convert
    /// the timestamp back to a unix epoch.
    pub fn unix_timestamp(&self) -> Duration {
        parts_as_duration(self.seconds(), self.fractional()) + CELESTICA_EPOCH
    }

    #[inline]
    /// The celestica timestamp as a duration.
    ///
    /// NOTE:
    /// This does not add the original [CELESTICA_EPOCH] back to the duration.
    pub fn celestica_timestamp(&self) -> Duration {
    ...
    pub fn send(&mut self) -> Result<Self, TimestampError> {
        let ts = get_celestica_timestamp();

        let ts_old = self.celestica_timestamp();
    ...
    pub fn recv(&mut self, msg: &Self) -> Result<Self, TimestampError> {
        ...
        let ts_msg = msg.celestica_timestamp();
...
/// Get the current time since the [CELESTICA_EPOCH] as [Duration].
///
/// This timestamp is ensured to be accurate taking into account the
/// resolution lost when converting the timestamp.
pub fn get_celestica_timestamp() -> Duration {
    let duration = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();

    let (seconds, fractional) = duration_to_parts(duration - CELESTICA_EPOCH);
    parts_as_duration(seconds, fractional)
}
...
mod tests {
    ...
    fn test_unix_timestamp_conversion() {
        ...
        let dc_ts = get_celestica_timestamp();
        let converted_ts = dc_ts + CELESTICA_EPOCH;
    ...
    fn test_clock_drift_error() {
        ...
        let ts2 = HLCTimestamp::new(ts1.celestica_timestamp() + drift, 0, 1);
        assert!(matches!(ts1.recv(&ts2), Err(TimestampError::ClockDrift)));

        let mut ts = HLCTimestamp::new(ts1.celestica_timestamp() + drift, 0, 1);
    ...
    fn test_clock_overflow_error() {
        ...
        let ts2 = HLCTimestamp::new(ts1.celestica_timestamp(), u16::MAX, 1);
    ...
    fn test_timestamp_recv() {
        let mut ts1 = HLCTimestamp::now(0, 0);
        let mut ts2 = HLCTimestamp::new(ts1.celestica_timestamp(), 3, 1);
    ...
    fn test_timestamp_ordering() {
        ...
        let mut ts1 = HLCTimestamp::now(0, 0);
        let ts2 = HLCTimestamp::new(ts1.celestica_timestamp(), 1, 1);
